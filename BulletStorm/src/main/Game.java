...
public class Game extends BasicGame {
    ...
    static boolean showHitbox = true;
    ...
    public void init(GameContainer gc) throws SlickException {
        ...
        for (int i = 0; i < 1; i += 1) {
            Enemy enemy = new Enemy(enemySprite, i, 400, 30, 30);
    ...
    public void render(GameContainer gc, Graphics g) throws SlickException {
        ...
        for (GameObject tile : tile_list) {
            ...
            tile.getImg().draw(tile.getX(), tile.getY());
            if (showHitbox) {
        ...
        for (Enemy enemy : enemy_list) {
            ...
            g.setColor(Color.red);
            enemy.getImg().draw(enemy.getX(), enemy.getY());
            if (showHitbox) {
                
            	g.draw(enemy.getHitbox());
            }
        }
        ...
        for (Bullet bullet : bullet_list) {
            ...
            g.setColor(Color.red);
            bullet.getImg().draw(bullet.getX(), bullet.getY());

            if (showHitbox) {
                g.draw(bullet.getHitbox());
            }
        ...
        g.drawString("time: "+ (System.currentTimeMillis()), 10, 150);
